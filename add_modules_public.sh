#!/bin/bash
# Same as add_modules.sh, but skips submodules whose remote repo is private
# (or otherwise inaccessible with the current GitHub token).
#
# Detection uses the GitHub REST API via `gh api`. If the repo returns
# .private == false we add it; for any other result (private / 404 / network
# error / no auth) we skip with a warning.
...
# is_public_repo URL  ->  returns 0 if the repo is reachable AND not private
is_public_repo() {
    local url="$1"
    # Extract "owner/repo" from a GitHub https URL, stripping a trailing .git
    local slug
    slug=$(echo "$url" \
        | sed -E 's#^https?://[^/]+/##; s#\.git$##; s#/+$##')

    if [[ -z "$slug" || "$slug" != */* ]]; then
        echo "  ! could not parse owner/repo from $url" >&2
        return 1
    fi

    # Prefer `gh api` (uses GITHUB_TOKEN automatically).
    if command -v gh >/dev/null 2>&1; then
        local priv
        priv=$(gh api "repos/$slug" --jq '.private' 2>/dev/null)
        case "$priv" in
            false) return 0 ;;     # public
            true)  return 1 ;;     # private
            *)     ;;              # 404 / network error -> fall through
        esac
    fi

    # Fallback: unauthenticated HEAD on the html URL. A public repo returns 200,
    # private/missing returns 404.
    local code
    code=$(curl -s -o /dev/null -w '%{http_code}' -I "https://github.com/$slug")
    [[ "$code" == "200" ]]
}
...
for entry in "${modules[@]}"; do
    ...
    echo "Checking submodule: $path  ($url @ $branch)"

    if ! is_public_repo "$url"; then
        echo "  >> SKIP (private / inaccessible): $url"
        skipped+=("$path  ($url)")
        echo
        continue
    fi
    ...
    if git submodule add -b "$branch" "$url" "$path"; then
        ...
        echo "  !! git submodule add failed for $path"
        skipped+=("$path  (add failed)")
    fi
    echo
done
